...
#define PICC_REQIDL \
  0x26  // REQuest command, Type A. Invites PICCs in state IDLE to go to READY
        // and prepare for anticollision or selection. 7 bit frame.
#define PICC_REQALL \
  0x52  // Wake-UP command, Type A. Invites PICCs in state IDLE and HALT to go
...
#define PICC_READ \
  0x30  // Reads one 16 byte block from the authenticated sector of the PICC.
        // Also used for MIFARE Ultralight.
#define PICC_WRITE \
  0xA0  // Writes one 16 byte block to the authenticated sector of the PICC.
