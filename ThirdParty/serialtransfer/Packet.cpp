...
/*
  ...
  * void
*/
void Packet::debugPrint(const char* str) {
  if (debug && debugPort != NULL) {
    HAL_UART_Transmit(debugPort, (uint8_t*)str, strlen(str), 100);
  }
}
...
/*
 void Packet::begin(const bool& _debug, UART_HandleTypeDef* _debugPort, const
 uint32_t& _timeout) Description:
 ------------
  * Simple initializer for the Packet Class
 Inputs:
 -------
  * const bool& _debug - Whether or not to print error messages
  * UART_HandleTypeDef* _debugPort - UART handle for debug output
  ...
  * void
*/
void Packet::begin(const bool& _debug, UART_HandleTypeDef* _debugPort,
...
/*
 uint8_t Packet::constructPacket(const uint16_t& messageLen, const uint8_t&
 packetID) Description:
 ------------
  * Calculate, format, and insert the packet protocol metadata into the packet
 transmit buffer Inputs:
 -------
...
*/
uint8_t Packet::constructPacket(const uint16_t& messageLen,
...
uint8_t Packet::parse(const uint8_t& recChar, const bool& valid) {
  bool packet_fresh =
      (packetStart == 0) || ((HAL_GetTick() - packetStart) < timeout);

  if (!packet_fresh)  // packet is stale, start over.
  {
    if (debug) debugPrint("ERROR: STALE PACKET\r\n");
    ...
    switch (state) {
      ...
      case find_payload_len:  ////////////////////////////////////////
      {
        if ((recChar > 0) && (recChar <= MAX_PACKET_SIZE)) {
          ...
          if (debug) debugPrint("ERROR: PAYLOAD_ERROR\r\n");
      ...
      case find_crc:  ///////////////////////////////////////////
      {
        ...
        if (calcCrc == recChar)
          ...
          if (debug) debugPrint("ERROR: CRC_ERROR\r\n");
      ...
      case find_end_byte:  ///////////////////////////////////////////
      {
        ...
        if (recChar == STOP_BYTE) {
          ...
          if (callbacks) {
            if (idByte < callbacksLen)
              ...
              char logBuf[64];
              sprintf(logBuf,
                      "ERROR: No callback available for packet ID %d\r\n",
                      idByte);
              debugPrint(logBuf);
            }
          }
        ...
        if (debug) debugPrint("ERROR: STOP_BYTE_ERROR\r\n");
      ...
      default: {
        if (debug) {
          char logBuf[32];
          sprintf(logBuf, "ERROR: Undefined state %d\r\n", state);
          debugPrint(logBuf);
        }
...
*/
uint8_t Packet::currentPacketID() { return idByte; }
