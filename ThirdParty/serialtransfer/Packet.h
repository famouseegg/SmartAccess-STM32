...
|      |
|______|____________________________________________________________Packet ID (0
by default)
|______|_____________________________________________________________________Start
byte (constant)
*/
...
const uint8_t MAX_PACKET_SIZE =
    0xFE;  // Maximum allowed payload bytes per packet
...
class Packet {
 public:  // <<---------------------------------------//public
  ...
  void begin(const bool& _debug = true, UART_HandleTypeDef* _debugPort = NULL,
             const uint32_t& _timeout = DEFAULT_TIMEOUT);
  uint8_t constructPacket(const uint16_t& messageLen,
                          const uint8_t& packetID = 0);
  uint8_t parse(const uint8_t& recChar, const bool& valid = true);
  uint8_t currentPacketID();
  void reset();

  /*
   uint16_t Packet::txObj(const T &val, const uint16_t &index=0, const uint16_t
   &len=sizeof(T)) Description:
   ------------
   ...
   Return:
   -------
    * uint16_t maxIndex - uint16_t maxIndex - Index of the transmit buffer
   (txBuff) that directly follows the bytes processed by the calling of this
   member function
  */
  template <typename T>
  uint16_t txObj(const T& val, const uint16_t& index = 0,
  ...
  /*
   uint16_t Packet::rxObj(const T &val, const uint16_t &index=0, const uint16_t
   &len=sizeof(T)) Description:
   ------------
   ...
   Return:
   -------
    * uint16_t maxIndex - Index of the receive buffer (rxBuff) that directly
   follows the bytes processed by the calling of this member function
  */
  template <typename T>
  uint16_t rxObj(const T& val, const uint16_t& index = 0,
                 const uint16_t& len = sizeof(T)) {
    uint8_t* ptr = (uint8_t*)&val;
    uint16_t maxIndex;
